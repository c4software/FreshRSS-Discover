# TASKS.archive.md — Closed Goals

The detail of the Goals that are entirely `[x]`: their tasks, the decisions
taken and their reason, the debts they opened. Moved here at closing so that
[TASKS.md](./TASKS.md) — the live memory — stays short enough to be read at
every session.

**This file is history, not state.** It is not audited again by `/verify`, and
no task is taken from it. The current state lives in `TASKS.md`; the rules in
[AGENTS.md](./AGENTS.md); the what in [SPECS.md](./SPECS.md); the how in
[ARCHITECTURE.md](./ARCHITECTURE.md).

Goals appear in the order they were closed.
...
- [x] `GOAL-038-T07` (2026-08-25) The framed look read as a band with a
      ...
      background. Capture `immersif-article-cadre` added
